...
- **Alerting:** Grafana's `data_quality.json` dashboard is built directly on `gold.quarantine_summary`, giving a concrete hook for a rule like "page on-call if quarantine volume exceeds 1% of total batch volume" — deliberately framed as pre-aggregated-and-dashboard-ready rather than requiring a separate alerting pipeline.
...
**Monitoring:** quarantined volume is aggregated nightly into `gold.quarantine_summary` by `batch_date`, `table_name`, and `failure_reason` — feeding the `data_quality.json` Grafana dashboard directly, so DQ monitoring is dashboard-native rather than requiring log-scraping.
